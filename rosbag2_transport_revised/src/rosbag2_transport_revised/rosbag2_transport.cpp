  ...
  const StorageOptions & storage_options, const PlayOptions & play_options)
{
  try {
    ...
    do {
      RCLCPP_INFO(this->transport_node_->get_logger(), "Open %s.", storage_options.uri.c_str());
      reader_->open(storage_options, {"", rmw_get_serialization_format()});
      RCLCPP_INFO(this->transport_node_->get_logger(), "Opened %s.", storage_options.uri.c_str());
